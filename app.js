...
/* In-memory fallback for Azad Studio */
...
/* Upstash REST first, ioredis fallback */
...
async function pushStudioPost(p) {
  try {
    const u = getUpstash();
    if (u) {
      const added = await u.sadd(AZAD_SEEN, String(p.id));
      if (added === 1) {
    ...
    pushStudioMem(p);
  } catch (e) { console.error("redis push err:", e.message); pushStudioMem(p); }
}
async function getStudioPosts() {
  try {
    ...
    return studioBuffer;
  } catch (e) { console.error("redis read err:", e.message); return studioBuffer; }
}
...
app.get("/api/gold", async (req, res) => {
  try {
    ...
    res.json({
      ...
      note: "Estimate based on recent trend"
    });
  } catch { res.json({ error: "unavailable" }); }
});
...
app.post("/telegram/webhook", express.json(), async (req, res) => {
  try {
    ...
    if (ch?.chat?.type === "channel") {
      ...
      if (Array.isArray(ch.photo) && ch.photo.length) { type = "photo"; file_id = ch.photo[ch.photo.length - 1].file_id; }
      if (ch.video) { type = "video"; file_id = ch.video.file_id; }
      const text = (ch.text || ch.caption || "");
      const entities = ch.entities || ch.caption_entities || [];
      const tags = entities.filter(e => e.type === "hashtag").map(e => text.slice(e.offset, e.offset + e.length));
      const firstLine = (text.split("\n")[0] || "").trim();
      const ist = new Date((ch.date || Math.floor(Date.now()/1000)) * 1000).toLocaleString("en-IN", {
        timeZone: "Asia/Kolkata", day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit", hour12: true
      });
      const title = firstLine ? firstLine.slice(0, 100) : `${type === "video" ? "Video" : type === "photo" ? "Photo" : "Post"} • ${ist}`;

      await pushStudioPost({ id: ch.message_id, type, file_id, title, caption: text, date: ch.date, tags });
    ...
    if (msg?.chat?.type === "private") {
      let type = "text", file_id = null;
      if (Array.isArray(msg.photo) && msg.photo.length) { type = "photo"; file_id = msg.photo[msg.photo.length - 1].file_id; }
      if (msg.video) { type = "video"; file_id = msg.video.file_id; }
      const text = (msg.text || msg.caption || "");
      await pushStudioPost({ id: msg.message_id, type, file_id, title: (text.split("\n")[0] || "").slice(0, 100), caption: text, date: msg.date, tags: [] });
      return res.sendStatus(200);
    }

    return res.sendStatus(200);
  } catch (e) { console.error("Webhook error:", e?.message); return res.sendStatus(200); }
});
...
app.get("/api/redis/where", async (req, res) => {
  ...
  try {
    if (type === "upstash-rest") {
      const u = getUpstash(); ping = await u.ping();
    } else if (type === "ioredis") {
      const r = await getRedis(); if (r) ping = await r.ping();
    } else { ping = "ok"; }
